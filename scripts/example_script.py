...
zone_level = "bezirk"
gdf = zone_getters.PopulationGetter(
    db=db, zone_level=zone_level
).get_result()  # gets a geopandas dataframe with various info -- super fast because SQL query behind
...
zone_level = "zaehlsprengel"
gdf = zone_getters.PopulationDensityGetter(
    db=db, zone_level=zone_level, simplified=True
).get_result()  # gets a geopandas dataframe with various info -- super fast because SQL query behind
...
gdf = zone_getters.PopulationGetter(
    db=db, zone_level=zone_level, simplified=False
).get_result()  # gets a geopandas dataframe with various info -- super fast because SQL query behind
